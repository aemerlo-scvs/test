package com.scfg.core.adapter.persistence.mortgageReliefValidations.insurancePendingApproval;
...
import com.scfg.core.application.port.out.mortgageReliefValidations.InsurancePendingApprovalValidationPort;
...
public class InsurancePendingApprovalValidationAdapter implements InsurancePendingApprovalValidationPort {
    ...
    @Override
    public ValidationResponseDTO insurancePendingApprovalDHL(Long operationNumber, String ci, Long insurancePolicyHolder) {
        ...
        if (subscriptionReportJpaEntityList.size() > 0) {
            ...
            if (subscription.getNsubscriptionStatus().equals("ACTIVO")) {
        ...
        if (subscriptionTrackingJpaEntityList.size() > 0) {
            ...
            if (subscriptionTracking.getInsuranceRequest().getRequestStatus().getId().equals(HelpersConstants.ACTIVE)) {
                return new ValidationResponseDTO(true, null);
            }
        }
        if (manualCertificateJpaEntityList.size() > 0) {
            ...
            if (manualCertificate.getInsuranceRequest().getRequestStatus().getId().equals(HelpersConstants.ACTIVE)) {
                return new ValidationResponseDTO(true, null);
            }
            return new ValidationResponseDTO(false, VR_INSURANCE_PENDING_APPROVAL);
        }
        return new ValidationResponseDTO(false, VR_INSURANCE_NOT_FOUND);
    }

    @Override
    public ValidationResponseDTO insurancePendingApprovalDHN(Long operationNumber, String ci, Long insurancePolicyHolder) {
        List<SubscriptionReportJpaEntity> subscriptionReportJpaEntityList = subscriptionReportRepository
                .findAllByHdocumentNumberAndDoperationNumber(ci, operationNumber + "");
